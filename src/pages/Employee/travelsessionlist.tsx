...
export default function AttendanceList() {
  ...
  // Only the slices actually used here are selected, so this component
  ...
  // ---------------------------------------------------------------------
  // Local, UI-only state (not worth sharing globally)
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // Current user info (role/department/area) -> pushed into the store once
  // ---------------------------------------------------------------------
  useEffect(() => {
    try {
  ...
  // Initial fetch - the store's TTL cache means this is a no-op if data is
  // already fresh (e.g. navigating back to this page).
  useEffect(() => {
    if (currentUserInfo) {
      loadAllSessions();
    }
  ...
  // ---------------------------------------------------------------------
  // Derived data (computed, not stored - no extra renders/state to manage)
  // ---------------------------------------------------------------------
  ...
  }, [startDate, endDate, selectedUser, appliedSearch, travelSessions]);
  ...
  // Wrapper so JSX call-sites don't need to pass sessionLogs/travelSessions/sessionsMap every time.
  ...
  // ---------------------------------------------------------------------
  // Auto-refresh: uses the lightweight active-sessions poll, not a full reload.
  // ---------------------------------------------------------------------
  useEffect(() => {
    if (autoRefresh) {
  ...
  const handleSearchSubmit = () => setAppliedSearch(searchQuery);
  ...
  const openMultiSessionMap = useCallback(
    async (group: GroupedSession) => {
      ...
      // Only fetch logs for sessions we don't already have cached.
      await Promise.all(
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                      ...
                      <div className="bg-white/5 dark:bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 border border-white/10 dark:border-gray-700/50">
                        ...
                        <p className="text-sm font-semibold text-gray-800 dark:text-white">
                          {formatTimeOnly(
                            group.sessions[group.sessions.length - 1].endTime ||
