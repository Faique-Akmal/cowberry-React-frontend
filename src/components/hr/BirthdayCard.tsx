...
interface User {
  ...
  role: string;
  is_checkin: boolean;
  birthDate: string; // This is the birthday date
  department: string;
  address: string;
}
...
const BirthdayCardList: React.FC = () => {
  ...
  // Check if today is the birthday using the "birthDate" field
  const isBirthdayToday = (birthDateString: string): boolean => {
    ...
    if (isNaN(birthDate.getTime())) {
      // console.warn('Invalid birthDate:', birthDateString);
      return false;
    }
    ...
    const loadBirthdayUsers = async () => {
      try {
        ...
        if (result.success) {
          ...
          // Filter users with birthdays today using the "birthDate" field
          const todayBirthdays = users.filter((user: User) => {
            if (!user.birthDate) {
              return false;
            }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-40">
        <div className="absolute -top-40 -right-40 w-80 h-80 rounded-full bg-gradient-to-r from-purple-500/20 to-pink-500/20 blur-3xl animate-pulse"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full bg-gradient-to-r from-yellow-500/20 to-orange-500/20 blur-3xl animate-pulse" style={{animationDelay: '1s'}}></div>
      </div>
      ...
      <div className="relative z-10">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center p-4 rounded-2xl backdrop-blur-sm mb-4"
            style={{
              background: isDarkMode 
                ? 'rgba(139, 92, 246, 0.2)' 
                : 'rgba(139, 92, 246, 0.1)',
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <div className="relative inline-block mb-6">
              <div className="w-24 h-24 rounded-2xl backdrop-blur-sm flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300"
                style={{
                  background: isDarkMode 
              ...
              <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-gradient-to-r from-purple-400 to-pink-400 flex items-center justify-center text-white text-sm shadow-lg">
                ?
              </div>
            </div>
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-4">
                    ...
                    <div className="relative flex-shrink-0">
                      <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white text-xl font-bold shadow-lg backdrop-blur-sm group-hover/card:scale-110 transition-transform duration-300"
                        style={{
                      ...
                      <div className="absolute -bottom-2 -left-2 w-6 h-6 rounded-full bg-gradient-to-r from-yellow-400 to-orange-400 flex items-center justify-center text-xs font-bold text-white shadow-md">
                        {age}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2 mb-3">
                        ...
                        <span className="text-xs px-3 py-1 rounded-full backdrop-blur-sm truncate max-w-[140px]"
                          style={{
                            background: isDarkMode 
                              ? 'rgba(139, 92, 246, 0.2)' 
                              : 'rgba(139, 92, 246, 0.1)',
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span className="text-xs px-3 py-1 rounded-full backdrop-blur-sm flex items-center gap-1"
                          style={{
                            background: isDarkMode 
                              ? 'rgba(16, 185, 129, 0.2)' 
                              : 'rgba(16, 185, 129, 0.1)',
        ...
        <div className="mt-6 pt-6 border-t border-gray-200/50 dark:border-gray-700/50">
          <div className="flex items-center justify-center gap-2 text-sm text-gray-600 dark:text-gray-400 backdrop-blur-sm p-3 rounded-xl"
            style={{
              background: isDarkMode 
                ? 'rgba(255, 255, 255, 0.05)' 
                : 'rgba(0, 0, 0, 0.02)',
