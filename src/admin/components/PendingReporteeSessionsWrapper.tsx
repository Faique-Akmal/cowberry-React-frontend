...
const PendingReporteeSessionsWrapper = () => {
  ...
  // captured before ReporteeTravelSessionManager ever got a chance to open
  // it. Gating on state?.sessionId !== undefined means our own clear is
  // ignored, while a genuinely new click (which always carries a
  // sessionId) still updates the ref - so clicking a second pending
  // session while this route stays mounted also works, and no artificial
  // delay is needed.
  if (
