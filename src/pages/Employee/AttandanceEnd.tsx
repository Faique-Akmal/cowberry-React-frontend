    ...
    const checkSubmitted = async () => {
      try {
        ...
        const res = await API.get(
          `/attendance-end/?user=${user.id}&date=${today}`,
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.odometer_image) {
        data.append(
          "odometer_image",
          formData.odometer_image,
          formData.odometer_image.name,
        );
      }
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        localStorage.setItem(
          `attendance_${formData.user}_${today}`,
          "submitted",
        );
        setAlreadySubmitted(true);
    ...
    >
      ...
      >
        {locationFetched
          ...
          : t("attendence.📍Click to End Attendance")}
      </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {alreadySubmitted
            ? t("attendence.Already Submitted")
            : loading
              ? t("attendence.Submitting...")
              : t("attendence.✅ Submit Attendance")}
        </button>
      </form>
    </div>
  );
